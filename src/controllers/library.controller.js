...
const viewMediaForPlaylist1 = async (req, res) => {
  try {
    const mediafiles = await library.viewMedia();
    ...
    res.render("editPlaylist", { mediafiles: convertedMediaFiles });
  } catch (err) {
    console.error('Error in viewMediaForPlaylist1:', err);
    res.status(500).send("Error viewing media for Playlist");
  }
};
...
module.exports = {
  ...
  deleteMedia,
  intervalToString,
  viewMediaForPlaylist,viewMediaForPlaylist1
};
