...
const getdataPlaylist = async () => {
  try {
    const result = await db.query("SELECT * FROM playlists1235 ORDER BY id DESC");
    return result.rows;
  } catch (err) {
    console.error("Error occurred at fetching all screens:", err);
    throw err;
...
module.exports = {
  ...
  deletePlaylist,
  deleteScreenById,
  screenByName,getStatus,getOnlineCountByClientTable,getOfflineCountByClientTable,getdataPlaylist
};
